 ...
 * @Last Modified time: 2019-06-08 21:11:09
 */
...
export default class BhvKeyboard extends cc.Component {
    ...
    /**键盘唯一实例 */
    static Instance:BhvKeyboard = null;
    ...
    onLoad () {
        this._timer =0;
        BhvKeyboard.Instance = this;
        if(this.node.name !== 'Canvas'){
            cc.error('BhvKeyBoard 组件 必须挂载 全局 Canvas 下才能使用')
        }
    }
    ...
    static AddKeyUp(key:cc.macro.KEY|string, callback:(duration?:number)=>void, target?: any, useCapture?: boolean){
        ...
        BhvKeyboard.Instance.events.on(SIGNAL.KEY_UP + cc.macro.KEY[key],callback,target,useCapture);
    }

    /**增加一个键位按下监听 */
    static AddKeyDown(key:cc.macro.KEY|string, callback:(duration?:number)=>void, target?: any, useCapture?: boolean){
        key = BhvKeyboard.ToMacroKey(key);
        BhvKeyboard.Instance.events.on(SIGNAL.KEY_DOWN + cc.macro.KEY[key],callback,target,useCapture);
    }
    
    /**按键松开，绑定只触发一次的回调 */
    static AddKeyUpOnce(key:cc.macro.KEY|string, callback:(duration?:number)=>void, target?: any){
        key = BhvKeyboard.ToMacroKey(key);
        BhvKeyboard.Instance.events.once(SIGNAL.KEY_UP + cc.macro.KEY[key],callback,target);
    }
    /**按键按下，绑定只触发一次的回调 */
    static AddKeyDownOnce(key:cc.macro.KEY|string, callback:(duration?:number)=>void, target?: any){
        key = BhvKeyboard.ToMacroKey(key);
        BhvKeyboard.Instance.events.once(SIGNAL.KEY_DOWN + cc.macro.KEY[key],callback,target);
    }

    /**移除一个键位监听,只传递 key 将注销所有有关 key 的回调事件 */
    static RemoveKey(key:cc.macro.KEY|string, callback:Function, target?: any){
        key = BhvKeyboard.ToMacroKey(key);
        BhvKeyboard.Instance.events.off(SIGNAL.KEY_DOWN + cc.macro.KEY[key],callback,target);
        BhvKeyboard.Instance.events.off(SIGNAL.KEY_UP + cc.macro.KEY[key],callback,target);
    }
    ...
    static RemoveAllKeys(target:any){
        //移除一个 key
        BhvKeyboard.Instance.events.targetOff(target);
    }
    ...
    static GetKeyDuration(key:cc.macro.KEY|string){
        key = BhvKeyboard.ToMacroKey(key);
        if(!this.Instance.keyMap.get(key))return 0;

        let start =  this.Instance.keyTimes.get(key);
        if(start == undefined || start > this.Instance._timer)return 0;

        let duration = this.Instance._timer - start;
    ...
    static GetLastKey():cc.macro.KEY{
        return  this.Instance.triggerKey;
    }
    ...
    static GetLastKeyName():string{
        return BhvKeyboard.GetKeyName(this.Instance.triggerKey);
    }
    ...
    static SimKeyUp(key:cc.macro.KEY|string){
        key = BhvKeyboard.ToMacroKey(key);
		if (this.Instance.keyMap.get(key)) return;
        this.Instance.keyMap.set(key,true);
        this.Instance.triggerKey = key;
        this.Instance._keyUp(key);
    }
    ...
    static SimKeyDown(key:cc.macro.KEY|string){
        key = BhvKeyboard.ToMacroKey(key);
		this.Instance.keyMap.set(key,false);
        this.Instance.triggerKey = key;
        this.Instance._keyDown(key);
    ...
    static GetKey(key:cc.macro.KEY|string):boolean{
        key = BhvKeyboard.ToMacroKey(key);
        return this.Instance.keyMap.get(key);
    }
